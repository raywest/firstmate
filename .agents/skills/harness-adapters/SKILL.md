...
| kimi | `--model <alias>` | `KIMI_MODEL_THINKING_EFFORT=<low\|high\|max>` env prefix, conditional | Model flag verified 2026-07-18 on kimi-code 0.27.0: `--model` is the long form of `-m`; a bad alias fails loudly with `config.invalid` before launch. Effort re-verified 2026-07-21 on 0.28.1: no CLI flag exists (`--effort`/`--thinking`/`--reasoning-effort` still all `unknown option`), but 0.28.1 added the documented `KIMI_MODEL_THINKING_EFFORT` env var, which forces the wire-level thinking effort per invocation regardless of the resolved model's own `default_effort`. `fm-spawn` maps low→low, medium/high/xhigh→high (kimi has no medium/xhigh tier), max→max, and emits the env prefix only when the resolved model's `config.toml` entry declares that value in `support_efforts` (the override bypasses that list rather than being validated against it, so an unlisted value reaches the provider live and 400s); when it cannot confirm support, fm-spawn falls back to today's record-only behavior (effort in meta, no override). See the kimi section below for the full verification evidence. |
...
## kimi (VERIFIED 2026-07-18, kimi-code 0.27.0; effort axis re-verified 2026-07-21 on 0.28.1; crewmate/scout duty on the tmux backend only)
...
| Effort | No CLI flag exists (reverified 2026-07-21 on 0.28.1: `--effort`, `--thinking`, `--reasoning-effort` all `unknown option`). 0.28.1 shipped `support_efforts`/`default_effort` per model in `config.toml` plus a documented `KIMI_MODEL_THINKING_EFFORT` env var that forces the wire-level effort per invocation, bypassing the model's declared list rather than being validated against it. Verified live 2026-07-21: a `low` launch's wire log recorded `"thinkingEffort":"low"` (18 output tokens), a `max` launch recorded `"thinkingEffort":"max"` (31 output tokens, visible reasoning trace), and an out-of-list value (e.g. `medium` on a model whose `support_efforts` is `["low","high","max"]`) reached the provider live and returned `provider.api_error: 400 Invalid request Error`. `fm-spawn` therefore emits `KIMI_MODEL_THINKING_EFFORT=<mapped>` as an env prefix only when the resolved model's config entry lists the mapped value; otherwise it falls back to record-only in meta, exactly as before this axis existed. A model-alias approach (two `config.toml` aliases pointing at the same underlying model with different `default_effort`) was also confirmed viable via `kimi doctor config` on a scratch `KIMI_CODE_HOME`, but the env var needs no config.toml write, no lock, and no operator-defined aliases, so it is the mechanism `fm-spawn` uses. See the [launch-profile-axes table](#launch-profile-axes) for the low/medium/high/xhigh/max mapping. |
