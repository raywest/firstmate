...
Grok dodges its gate instead of granting trust, because its project picker appears only outside a project and the spawn starts in the task's git root.
...
This is rarely seen because a secondmate home is persistent and reused, so its trust decision is made once and survives, unlike a fresh disposable task worktree.
