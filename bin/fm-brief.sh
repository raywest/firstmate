...
When a decision you escalated is answered or a blocker clears and your domain resumes, append \`resolved [key=<slug>]: {how it was decided or unblocked}\` (using the same key if you opened it with one) so it is durably closed instead of resurfacing behind later unrelated events.
...
6. If a decision belongs to a human (product choices, destructive actions),
   ...
   When firstmate replies or a blocker clears and you resume, append \`resolved [key=<slug>]: {how it was decided or unblocked}\` (using the same key if you opened it with one) so the decision or blocker is durably closed and does not keep resurfacing.
7. Never stop, restart, or update the shared \`no-mistakes\` daemon - it is one instance serving
   every lane/home, so restarting it kills other lanes' in-flight pipeline runs. On ANY no-mistakes
   daemon error, append \`blocked: {the daemon error}\` and stop; only firstmate manages the daemon.
8. Deliver what was asked, at the scope intended. Make routine judgment calls yourself. If the task looks mistaken, or a better approach exists, say so in one line and continue with the task as asked rather than quietly narrowing, widening, or transforming it. Finish the whole task, and stop short of changes clearly beyond it.

# Definition of done
Write your findings to \`$DATA/$ID/report.md\`.
The report must stand alone: what you did, what you found, the evidence (commands run, output, file:line references), and what you recommend.
Match the report's length to what the task needs: cover the substance and the evidence, but do not pad with filler sections, redundant summaries, or boilerplate. A tight report that stands alone beats a long one.
...
2. Run \`no-mistakes doctor\`; if it reports the repo is not initialized here, run \`no-mistakes init\`."
    ...
    # A single apostrophe here (e.g. "firstmate's") makes bash's $(...) close-paren
    # scanner misparse the rest of the file (unbalanced-quote heredoc-in-command-
    # substitution bug), so this one heredoc uses a plain `read` instead of
    # `$(cat <<EOF...)` to stay immune to that apostrophe-parity trap.
