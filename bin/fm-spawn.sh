...
# Sourced only after the gate check above: fm-wake-lib.sh's module-scope
# `mkdir -p "$STATE"` must never run before a refusal that must leave no state
# directory behind.
...
# preserving compatibility with old metadata that has no backend= field.
...
launch_template() {
  ...
  case "$harness" in
    ...
    codex)
      if [ "$kind" = secondmate ]; then
        printf '%s' 'codex __MODELFLAG____EFFORTFLAG____HARNESSPROFILEFLAG__--dangerously-bypass-approvals-and-sandbox "$(__OPINPUT__ encode launch-brief < __BRIEF__)"'
      else
        printf '%s' 'codex __MODELFLAG____EFFORTFLAG____HARNESSPROFILEFLAG__--dangerously-bypass-approvals-and-sandbox -c "notify=[\"bash\",\"-c\",\"touch __TURNEND__\"]" "$(__OPINPUT__ encode launch-brief < __BRIEF__)"'
      fi
      ;;
    ...
    # __EFFORTFLAG__ here carries a KIMI_MODEL_THINKING_EFFORT=<level> env
    # prefix instead of a trailing arg (effort_flag_for_harness's kimi case),
    # so it sits before the binary path rather than after the flags.
...
# on any other backend are unverified paths - refuse loudly instead of launching a
  ...
  case "$HARNESS" in
    kimi*)
      ...
      if [ "$BACKEND" != tmux ]; then
        echo "error: kimi spawns are verified on the tmux backend only; backend=$BACKEND is unverified for kimi." >&2
        exit 1
      fi
      ;;
  esac
...
effort_flag_for_harness() {
  ...
  case "$harness" in
    ...
    kimi)
      ...
      # this effort. Absent that proof (no matching block, no support_efforts, a
      # missing config, or the value not listed), fall back to today's behavior -
      # record-only in meta, no override, launch unchanged.
...
# Session-provider container-ensure + task creation. The tmux adapter owns
# session selection and task-window creation (see bin/backends/tmux.sh); a
# herdr spawn goes through the version-gated, workspace-per-HOME,
