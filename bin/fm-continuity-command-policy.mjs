...
// fm-teardown.sh's fail-closed (non-forced) teardown is also a recovery action;
// unsafeTeardown below still blocks a forced call regardless of set membership.
const RECOVERY_SCRIPTS = new Set(["fm-wake-drain.sh", "fm-watch-arm.sh", "fm-daemon-launch.sh", "fm-teardown.sh"]);
