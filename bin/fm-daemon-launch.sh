...
#
# Usage:
#   fm-daemon-launch.sh start     Capture the captain pane, then (unless the daemon
...
FM_DAEMON_LAUNCH_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FM_ROOT="${FM_ROOT_OVERRIDE:-$(cd "$FM_DAEMON_LAUNCH_DIR/.." && pwd)}"
...
. "$FM_DAEMON_LAUNCH_DIR/fm-backend.sh"
# shellcheck source=bin/fm-supervisor-target-lib.sh
. "$FM_DAEMON_LAUNCH_DIR/fm-supervisor-target-lib.sh"
# fm-afk-start.sh provides fm_afk_clear_stale_artifacts and, via its own
# bin/fm-wake-lib.sh source, the daemon-lock liveness helpers (homed beside
# fm_watcher_healthy). It is sourceable (BASH_SOURCE guard) and its main does
# not run on source. It sets `set -eu`, so turn errexit back off for this
# script's best-effort flow immediately after.
# shellcheck source=bin/fm-afk-start.sh
. "$FM_DAEMON_LAUNCH_DIR/fm-afk-start.sh"
set +e
...
fm_afk_launch_herdr_recover_created() {  # <session> <label>
  local session=$1 label=$2 workspaces ws_count wsid panes pane_count pane
...
fm_afk_launch_daemon_cmd() {  # <captain-target> <captain-backend> <entry>
  local captain_target=$1 captain_backend=$2 entry=$3 config_dir daemon_env x_mode_env
  config_dir="${FM_CONFIG_OVERRIDE:-$FM_HOME/config}"
  daemon_env="$config_dir/daemon.env"
  x_mode_env="$config_dir/x-mode.env"
