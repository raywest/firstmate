...
fm_backend_tmux_agent_state() {  # <target>
  ...
  comm=${comm#-}
  case "$comm" in
    # Kimi is intentionally absent: it is verified for crewmate/scout duty only
    # and cannot be a secondmate, so this recovery-grade liveness probe never
    # attributes a live agent process to it.
