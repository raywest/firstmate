...
fm_tmux_composer_state() {  # <target> -> empty|pending|pending-unproven|unknown
  ...
  local row row_raw state unknown_seen=0 cursor_trimmed
  ...
  if box=$(fm_tmux_find_composer_box "$cy" "$plain"); then
    ...
    # box_status 3: the cursor row alone has a composer edge, but no top or
    # bottom border row was found ANYWHERE in the pane (never a crossed or
    # unclosed multi-row box - that is box_status 2, always unknown above). A
    # single row that opens and closes with the SAME plain vertical border in
    # that shape (e.g. kimi's inline `| > |`) is a complete, self-bordered
    # one-row box. Resolved from the SAME full-pane capture already used for
    # box-finding (never a fresh re-capture), so a changed pane between reads
    # cannot admit an edge the box scan never saw.
    if [ "$box_status" -eq 3 ]; then
      cursor_trimmed=$(printf '%s\n' "$plain" | sed -n "$((cy + 1))p")
      cursor_trimmed="${cursor_trimmed#"${cursor_trimmed%%[![:space:]]*}"}"
      cursor_trimmed="${cursor_trimmed%"${cursor_trimmed##*[![:space:]]}"}"
      case "$cursor_trimmed" in
        '│'*'│'|'┃'*'┃'|'║'*'║'|'|'*'|')
          row_raw=$(printf '%s\n' "$pane" | sed -n "$((cy + 1))p")
          fm_tmux_composer_row_state "$row_raw" 1 0
          return 0
          ;;
      esac
      printf 'unknown'
