...
`cycle_begin` snapshots `state/.wake-queue.seq` when it starts owning or following a pid; at cycle end, if that counter advanced, a real wake landed durably even though this process could not see its reason text (an attached arm never captures the owning watcher's stdout, and an owned child can die between a successful `fm_wake_append` and its own `wake()` call).
...
Only a cycle whose wake-queue counter genuinely did not advance still emits the typed `watcher: FAILED - cycle ended without an actionable reason` result.
