...
The watcher and daemon share `bin/fm-classify-lib.sh` for captain-relevant status verbs, declared-external-wait vocabulary, status-scan primitives, and the provably-working guard for no-verb signals.
The always-on watcher also uses that library's absorb classification on first-sighting stale panes before status-log terminality is trusted, while the daemon retains its afk stale-persistence cadence and exposes a test-only present-mode branch that promptly escalates a stopped crew on first sight.
