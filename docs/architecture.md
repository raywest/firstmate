...
[`alwayson-triage.md`](alwayson-triage.md#single-resolver-escalation-authority) is the single owner of the shared no-verb and stale-pane classification policy, including positive working evidence and bounded pause or absorption rechecks.
...
That block owns the live wait shape for the running primary harness: a claude or codex primary on tmux or herdr collapses to the always-on triage daemon's marked-message protocol (below and [`alwayson-triage.md`](alwayson-triage.md)); every other harness/backend combination keeps its per-wake protocol - Grok uses background-notify cycles, Codex on any other backend uses bounded foreground checkpoints, Pi uses its two tracked primary extensions, and OpenCode uses its TUI plugin.
`bin/fm-watch-arm.sh` remains the verified arm wrapper for protocols that call it; it forks the watcher as a tracked child, verifies it is genuinely alive with a fresh liveness beacon, and prints an honest `started` or `attached` status, or a typed nonzero result.
On `attached` it stays live across identity-matched successors, and an unexplained clean child close either attaches to a verified healthy successor or becomes a typed nonzero result: the loud `watcher: FAILED - cycle ended without an actionable reason` only when the wake queue genuinely did not advance during the cycle, an accurate already-queued report otherwise (see [`watcher-continuity.md`](watcher-continuity.md)).
...
Unflipped Claude harness/backend combinations retain the tracked background-task protocol and its narrow PreToolUse continuity gate, which allows drain, arm recovery, daemon launch recovery, and fail-closed teardown while refusing only other fleet commands when tasks are in flight and neither an identity-matched live watcher nor a live daemon holds the home lock.
...
On every verified primary harness, tracked hook integration gives the primary session a push-based backstop: when work is in flight and neither an identity-matched watcher lock with a fresh beacon nor a live daemon lock is held, direct Stop hooks block and passive turn-end hooks force one bounded follow-up.
...
[`alwayson-triage.md`](alwayson-triage.md#single-resolver-escalation-authority) is the single owner of the shared stale-pane and no-verb classification policy, including its terminal-verb and away-mode wedge-aging refinements.
...
The dispatch file is intentionally judgment-based: firstmate reads the natural-language rules at intake, chooses the best matching rule, resolves profile arrays itself from current quota output under `AGENTS.md` section 4, and passes only concrete `--harness`, `--model`, `--effort`, and `--harness-profile` axes to `fm-spawn.sh`.
...
The schema and axis semantics are owned by [configuration.md](configuration.md#crew-dispatch-profiles-configcrew-dispatchjson).
...
Its optional model, effort, and codex-only harness-profile pins, their four-token format, and precedence are owned by [configuration.md](configuration.md#harness-support).
Those pins are re-read on every secondmate spawn or respawn.
...
Local-only clones with an origin use the same guarded fetch-and-fast-forward path; only a local-only clone without an origin remains a benign local-only skip, while other originless clones and fetch failures retain their existing skips.
...
The sub-supervisor daemon (`bin/fm-supervise-daemon.sh`) is the permanent wake consumer on a supported claude or codex primary on tmux or herdr, reusing the same shared wake classifier as the always-on watcher; `/afk` only switches its delivery style. See [`alwayson-triage.md`](alwayson-triage.md) for the full mechanism.
