...
The shell scripts do not match those rules; firstmate chooses the best matching rule with judgment, resolves that rule directly or through a supported selector, and passes only concrete `--harness`, `--model`, `--effort`, and `--harness-profile` flags to `fm-spawn.sh`.
...
Kimi accepts the full shared vocabulary at profile validation; its per-model mapping and runtime support gate are documented in the [harness-adapters skill](../.agents/skills/harness-adapters/SKILL.md#kimi).
If `fm-spawn.sh` cannot apply a selected effort at launch, it records the requested `effort=` in task meta for traceability and omits the launch override.
