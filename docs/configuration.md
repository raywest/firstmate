...
Task meta records `backend=` only for a non-default backend; an absent `backend=` means `tmux`, preserving compatibility with existing default-backend metadata.
...
claude, codex, opencode, pi, and grok are empirically verified across their documented primary and crew roles.
`kimi` is empirically verified only for crewmate/scout dispatch on tmux; it is not a primary or secondmate harness, and `fm-spawn` refuses every other Kimi template launch shape.
`bin/fm-spawn.sh`'s header owns the raw launch-command exception to those template gates.
Before dispatching Kimi, initialize and authenticate it so `$KIMI_CODE_HOME/config.toml` exists; the doctor-validated hook setup and matching launch environment are owned by the harness-adapters skill.
New harnesses are verified through a supervised trial task before joining their supported scope.
...
The first non-empty, non-comment line is parsed as `<harness> [<model>] [<effort>] [<harness_profile>]`.
A bare `<harness>` preserves the previous behavior: harness only, with no model, effort, or harness-profile launch flag.
When the harness token is absent or `default`, secondmate launch falls back through `config/crew-harness` and then the primary's own harness, and no model, effort, or harness-profile token is read from that file.
`fm-harness.sh secondmate-model`, `fm-harness.sh secondmate-effort`, and `fm-harness.sh secondmate-harness-profile` expose only the optional tokens from `config/secondmate-harness`; `config/crew-harness` remains a bare adapter-name file.
An explicit harness argument to `fm-spawn.sh` still overrides either config file for that spawn only.
An explicit `--model`, `--effort`, or `--harness-profile` overrides the matching token from `config/secondmate-harness`; an explicit harness or raw launch command starts with clean model, effort, and harness-profile defaults unless those flags are also passed.
...
For template Kimi crews, `fm-spawn.sh` runs `fm-kimi-turnend-hook.sh install`, drops a per-task `.fm-kimi-turnend` pointer in the worktree, and records the matching private registry token and resolved Kimi home for teardown.
...
The shell scripts do not match those rules; firstmate chooses the best matching rule with judgment, resolves its profile object or array under the operating contract in `AGENTS.md` section 4, and passes only concrete `--harness`, `--model`, `--effort`, and `--harness-profile` flags to `fm-spawn.sh`.
When the file exists, `fm-spawn.sh` enforces that contract by refusing crewmate and scout spawns that lack an explicit harness (`--harness`, a positional adapter, or a raw launch command).
Batch spawns satisfy the same requirement with a shared `--harness`.
Secondmate spawns are exempt and still resolve through `config/secondmate-harness` and its optional model, effort, and harness-profile tokens.
...
```json
{
  ...
  "default": [
    { "harness": "<adapter>", "model": "<optional model>", "effort": "<optional effort>", "harness_profile": "<optional>" }
  ]
}
```
...
`use.model`, `use.effort`, `use.harness_profile`, and rule `why` are optional.
An omitted model, effort, or harness_profile means the selected harness uses its own default for that axis.
Every profile array is an implicit quota-aware choice.
If no dispatch rule fits, firstmate resolves `default` through the same object-or-array path before falling back to `config/crew-harness`.
`use.effort` uses the shared `low`, `medium`, `high`, `xhigh`, and `max` vocabulary, and bootstrap validates it against the selected harness's profile vocabulary rather than the selected model's configuration.
Kimi accepts the full shared vocabulary at profile validation; its per-model mapping and runtime support gate are documented in the [harness-adapters skill](../.agents/skills/harness-adapters/SKILL.md#kimi-verified-2026-07-25-kimi-0291-effort-axis-verified-2026-07-21-on-0281-crewmatescout-duty-on-the-tmux-backend-only).
...
Quota-aware selection is agent-owned (`AGENTS.md` section 4): firstmate resolves a matched profile array itself against current `quota-axi --json` output rather than delegating to a shell selector.
...
Malformed JSON, an empty or malformed rule/default array, an unverified harness, an unknown `select`, an effort value outside its harness's profile vocabulary, a malformed `harness_profile`, a `harness_profile` on a non-codex profile, or a `harness_profile` whose config file does not exist under `${CODEX_HOME:-$HOME/.codex}` is reported as `CREW_DISPATCH: invalid config/crew-dispatch.json - ...`; missing `jq` is reported through the normal `MISSING: jq` install-consent flow.
...
Normal completed runs keep a no-origin skip silent; a local-only project is silent only when it also has no origin, since one with a real origin gets the same fetch-and-fast-forward refresh as any other mode (see `fm-fleet-sync.sh`'s header for the local-only fetch bound and its drift-reporting behavior).
...
The poll atomically claims `state/x-context/<request_id>.offered.json` as pending before emitting that wake, re-emitting it until the watcher has durably queued it, and the watcher then marks the offer delivered.
Subsequent offers of a delivered request stay silent even after the inbox is drained following an answer or dismiss.
...
FM_CODEX_WATCH_CHECKPOINT=180   # seconds per foreground watcher checkpoint in Codex primary supervision on backends other than tmux or herdr
...
FM_STALE_ESCALATE_SECS=240         # seconds before a stale pane reaches its persistence recheck; see docs/alwayson-triage.md
FM_PAUSE_RESURFACE_SECS=3600       # seconds before an idle declared external wait re-surfaces for a recheck; see docs/alwayson-triage.md
FM_WEDGE_DEMAND_INSPECT_COUNT=3    # consecutive stale persistence escalations on the same unchanged pane before demand-deep-inspection is added
FM_WATCH_TRIAGE_LOG_MAX_BYTES=262144   # size cap for the watcher's absorbed-wake debug log
FM_FLEET_SYNC_BOOTSTRAP_TIMEOUT=     # optional seconds allowed for bootstrap's best-effort clone refresh; unset/blank defaults to max(20, 5 + 3 * origin-backed-project-count)
FM_FLEET_PRUNE=1        # set to 0 to skip pruning local branches whose upstream is gone
FM_FLEET_LOCAL_ONLY_FETCH_TIMEOUT_SECS=20  # seconds for a local-only-with-origin fetch only; 0 leaves that fetch unbounded
...
FM_BACKGROUND_WORK_REGEX='[0-9]+ (shells?|monitors?)\b.*\brunning\b'   # pane-tail regex for a harness-reported live background task; see docs/alwayson-triage.md
...
# sub-supervisor (bin/fm-supervise-daemon.sh); always-on, with state/.afk selecting delivery style
FM_SUPERVISOR_BACKEND=             # optional supervisor pane backend override; tmux/herdr only, otherwise detects $TMUX_PANE then HERDR_ENV/HERDR_PANE_ID before tmux fallback
FM_SUPERVISOR_TARGET=              # optional supervisor pane target override; tmux target or herdr <session>:<pane-id>, otherwise auto-detected
FM_INJECT_SKIP=heartbeat           # |-prefixes force-self-handled bypassing classification; empty disables
FM_ESCALATE_BATCH_SECS=90          # away-mode buffer window for batched escalation digests; 0 = flush immediately, regardless of urgency
FM_ESCALATE_BATCH_SECS_PRESENT=30  # present-mode routine-item window; buffered urgent items flush immediately
FM_MAX_DEFER_SECS=300              # max buffered escalation age before retry plus wedge alarm; 0 disables
FM_MAX_DEFER_SECS_PRESENT=900      # present-mode max buffered escalation age before retry plus durable wedge marker; 0 disables
