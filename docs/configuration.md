...
FM_BUSY_REGEX='esc (to )?interrupt|Working\.\.\.|Ctrl\+c: ?cancel'  # busy-pane signatures, shared by watcher, fm-crew-state pane fallback, and tmux helper; the optional space covers Kimi and Grok
...
KIMI_CODE_HOME=         # optional Kimi Code config home; defaults to ~/.kimi-code (see "Harness support")
