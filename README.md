...
- **Disposable worktrees by default** - tasks use clean [treehouse](https://github.com/kunchenguid/treehouse) git worktrees, or Orca-managed worktrees when `backend=orca`; projects can explicitly opt into [working in their real directory](bin/fm-spawn.sh) for volume-backed work that needs no scratch copy.
...
```
 ...
 └───┬────┘   └───┬────┘      └───┬────┘
     ▼            ▼               ▼
  treehouse/Orca worktree, declared real project directory, or isolated secondmate home
     │
