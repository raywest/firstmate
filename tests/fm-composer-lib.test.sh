...
test_matrix_herdr_halfblock_rule_bounds_bare_wrap() {
  ...
  local screen plain out
  # \xHH byte escapes, never \uHHHH: bash 3.2 (macOS's stock /bin/bash) does not
  # support \uHHHH in $'...', printf's format string, or printf '%b' - it passes
  # the literal backslash-u text through unchanged instead of the glyph. CI runs
  # on a bash with \uHHHH support, which is why this only broke here.
  ...
  screen=$'transcript\n \xe2\x96\x84\xe2\x96\x84\xe2\x96\x84\xe2\x96\x84\xe2\x96\x84\xe2\x96\x84\xe2\x96\x84\xe2\x96\x84\n'"  ${ESC_LOCAL}[2m\xe2\x86\x92 ${ESC_LOCAL}[0;7mA${ESC_LOCAL}[0;2mdd a follow-up${ESC_LOCAL}[0m"$'\n \xe2\x96\x80\xe2\x96\x80\xe2\x96\x80\xe2\x96\x80\xe2\x96\x80\xe2\x96\x80\xe2\x96\x80\xe2\x96\x80\n  Cursor Grok 4.5 High \xc2\xb7 6.7%   Run Everything\n  ~/wt \xc2\xb7 64cdd3a'
