...
# session_start_expected_session() <root>: bash-only reimplementation of
# bin/fm-backend-hometag-lib.sh's fm_backend_hometag for a primary home (no
# .fm-secondmate-home marker), mirroring tests/fm-backend-tmux.test.sh's
# identical tmux_expected_hometag helper. A FRESH tmux container-ensure (a
# real relaunch, not a stale recorded selector read verbatim) derives this
# hardened "fm-firstmate-<hash>" session name instead of the old bare literal.
session_start_expected_session() {  # <root>
  local root=$1 real hash
...
SH
  ...
  while [ "$i" -le 40 ]; do
    (
      # $BASHPID needs bash 4+; a backgrounded sentinel's real, live PID ($!) is
      # the portable bash-3.2-safe equivalent and, unlike a synthetic counter,
      # stays genuinely alive for fm_harness_pid_alive's real `kill -0` check.
      sleep 300 &
      harness_pid=$!
...
test_session_start_relaunches_missing_pi_secondmate() {
  local rec root home fakebin mate log spawned out first_calls second_calls session
...
EOF
  ...
  session=$(session_start_expected_session "$root")
  ...
  assert_contains "$out" "endpoint: alive (backend=tmux window=$session:fm-$SESSION_START_SECOND_MATE_ID)" \
...
test_session_start_preserves_proven_bare_shell_recovery() {
  local rec root home fakebin mate log spawned out session
