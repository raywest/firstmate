...
SH
  ...
  # Symlink, never cp: a COPY of a platform system binary (/bin/bash) loses its
  # code-signing trust on modern macOS and is SIGKILLed on exec (exit 137),
  # silently producing empty detection output. A symlink still execs the
  # original signed binary, so trust validation passes.
...
test_detects_versioned_process_ancestor() {
  ...
  for bin in muse-bin-0.1.0-R708.1 muse-bin-9.9.9-RZZZ.9 muse; do
    # Symlink, never cp: see the fixture comment in make_spawn_fakebin above.
