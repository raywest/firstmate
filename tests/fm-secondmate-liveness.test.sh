...
test_tmux_agent_state_classifies() {
  ...
  # Kimi is verified for crewmate/scout duty only and has no secondmate recovery
  # classifier, so a live kimi process must never be attributed as a recognized
  # live agent here - ambiguous is the safe fallback that licenses neither a
  # respawn (dead/missing only) nor a false "recognized live secondmate" read.
  fb=$(make_probe_tmux "$TMP_ROOT/tmux-kimi" kimi)
  out=$(PATH="$fb:$BASE_PATH" bash -c '. "$0/bin/fm-backend.sh"; fm_backend_agent_state tmux sess:win' "$ROOT")
  [ "$out" = ambiguous ] || fail "a live kimi foreground process should classify as ambiguous (no secondmate classifier), got '$out'"
